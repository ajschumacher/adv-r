...
Note that we need to be careful with more complicated expressions because user-created infix functions have very high precedence. They're higher in precedence every than other infix operator apart from `^`, `$`, `@` and `::`. For example, `x %<d-% a + b` is interpreted as `(x %<d-% a) + b`, so we need to use parentheses ourselves:
